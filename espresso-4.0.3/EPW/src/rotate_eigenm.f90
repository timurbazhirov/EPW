...
#include "f_defs.h"
  ...
  use el_phon, only : dynq, epmatq
  use phcom, only : nmodes
  use control_flags, only : iverbosity
  use pwcom, only : nbnd, nks, nr1, nr2, nr3, at, bg,amconv
  ...
  real(kind=DP), parameter :: eps = 1.d-10, zero = 0.d0, &
       twopi = 6.28318530717959, rydcm1 = 13.6058d0 * 8065.5d0
  !
  ! work variables 
  !
  ...
  IF ( iverbosity .eq. 1 ) then
    ...
    DO nu = 1, nmodes
      ...
      ENDIF
    ENDDO
    WRITE ( stdout, '(6(2x,f10.5) )' ) (wtmp(nu)*rydcm1*0.12398, nu = 1, nmodes)
    !
  ENDIF
  !
  !
  ...
  !
  ! in case time reversal is applied we use Eq. (4.4) Maradudin&Vosko, RMP
  !
  IF (timerev) cz2 = conjg ( cz2 )
  CALL errore('rotate_eigenm','rotate_eigenm: time reversal never tested',-1)
  !
